...
4. Data Modeling의 절차의 순으로 옳은 것은? 3번
...
5. Data Modeling의 절차에 대한 설명으로 옳은 것은? 4번
...
9. PK와 FK에 대해서 설명해 주세요.
 PK는개인의 주민등록번호와 같이 고유의 값이다. 한테이블당 하나씩 존재하며 부모의 역할을 한다.
NULL을 사용 할수 없다.
 FK는 외래 테이블로서 PK의 자식테이블을 의미하며PK에 연결을 하는 역할이다. PK에 데이터가 1,2,3이 존재하면
FK로 설정된 곳에는 1,2,3만 존재한다.
...
SELECT emp.emp_no, emp.birth_date, CONCAT(emp.first_name, ' ' , emp.last_name) fullname, emp.gender
FROM employees emp;
...
SELECT emp.birth_date, tit.title
FROM employees emp
...
CREATE INDEX mem_no on test (mem_no);
